...
Реле на чистом Python 3.12, без зависимостей: `relay.py` + systemd-юнит в песочнице + таймер копирования сертификата. Провайдеры по порядку: OpenRouter (бесплатные модели по списку `PRIORITY`, у думающих моделей рассуждения выключены) → Groq.

Список `PRIORITY` — три модели, отобранные прогоном 18.09.2026 на одном промпте: **DeepSeek V4 Flash** (лучший формат, ~7 с) → **Nemotron 3 Super** (~2 с, содержание верное, пункты не всегда) → **Ling 3.0 Flash** (~2 с). Все три — $0 за вход и выход по каталогу. Inkling Small отдаёт 403, Qwen3.8 27B, Gemma 4 и GLM 5.2 на бесплатном тарифе не отвечали — реле подменяло их на DeepSeek. Каталог живой, раз в пару месяцев стоит перепроверить тем же прогоном. Порт снаружи открыт **только для сетей Cloudflare** — правила ufw ставит `setup.sh`.
